...
export default function Consignors() {
  ...
  const canCreate = can(SLUGS.BILTY_CONSIGNORS_CREATE);
  const canUpdate = can(SLUGS.BILTY_CONSIGNORS_UPDATE);
  const canDelete = can(SLUGS.BILTY_CONSIGNORS_DELETE);

  const [items, setItems]           = useState<Consignor[]>([]);
  const [loading, setLoading]       = useState(true);
  const [search, setSearch]         = useState('');
  const [showForm, setShowForm]     = useState(false);
  const [editItem, setEditItem]     = useState<Consignor | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/v1/bilty-setting/consignors`);
      if (res.status === 401) { router.replace('/auth/login'); return; }
      const data = await res.json();
      setItems(data.consignors ?? data ?? []);
    } catch { setError('Failed to load consignors.'); }
  ...
  function openEdit(c: Consignor) {
    setEditItem(c);
    setForm({
      consignor_name:   c.consignor_name,
      gstin:            c.gstin ?? '',
      pan:              c.pan ?? '',
      mobile:           c.mobile ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const body: Record<string, string> = { consignor_name: form.consignor_name.trim() };
    (Object.keys(EMPTY_FORM) as (keyof typeof EMPTY_FORM)[])
      .filter(k => k !== 'consignor_name' && form[k].trim())
      .forEach(k => { body[k] = form[k].trim(); });

    try {
      const url = editItem
        ? `/v1/bilty-setting/consignors/${editItem.id}`
        : `/v1/bilty-setting/consignors`;
      ...
      setSuccess(editItem ? 'Consignor updated.' : 'Consignor created.');
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Delete consignor "${name}"?`)) return;
    setDeletingId(id); setError('');
    try {
      const res = await apiFetch(`/v1/bilty-setting/consignors/${id}`, {
        method: 'DELETE',      });
      if (!res.ok && res.status !== 204) { setError('Failed to delete.'); return; }
      setSuccess('Consignor deleted.'); fetchItems();
    } catch { setError('Unable to reach the server.'); }
    finally { setDeletingId(null); }
  }

  const filtered = items.filter(c =>
    c.consignor_name.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h1 className="text-xl font-bold text-slate-900 mb-1">Consignors</h1>
          <p className="text-sm text-slate-500">Manage consignors (senders) used in bilty creation.</p>
        </div>
        {canCreate && (
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
          <p className="text-sm font-semibold text-slate-700">{editItem ? 'Edit Consignor' : 'New Consignor'}</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {([
              ['consignor_name', 'Consignor Name *', 'text', true],
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((c, idx) => (
                <tr key={c.id ?? idx} className="hover:bg-slate-50 transition-colors">
                  <td className="px-5 py-3.5 font-medium text-slate-800">{c.consignor_name}</td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {canDelete && (
                          <button type="button" disabled={deletingId === c.id}
                            onClick={() => handleDelete(c.id, c.consignor_name)}
